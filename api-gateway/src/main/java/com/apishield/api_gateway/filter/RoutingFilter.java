...
public class RoutingFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        String apiName = request.getHeader("X-API-Name");
        String path = request.getRequestURI();

        if (apiName == null) {
            response.setStatus(HttpStatus.BAD_REQUEST.value());
            response.getWriter().write("Missing X-API-Name header");
            return;
        }

        String backendUrl = backendResolutionService.resolve(apiKey, apiName);
        if (backendUrl == null) {
